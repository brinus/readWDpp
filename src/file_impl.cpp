...
namespace readWDpp {
    file::impl::impl(const std::string& filename) 
        ...
        , _daq_type(daq_type::Undefined)
        , _boards()
        , _cur_bd_sn(0)
        , _cur_ch_idx(0)
        , _prev_header(header_type::UNDEFINED)
    {
        _is_init = initialize();
    }
    ...
    bool file::impl::_read_header() {
        ...
        if (header[0] == 'D' && header[1] == 'R' && header[2] == 'S') {
            if (version < 8 )       _daq_type = daq_type::DRS;
            else if (version == 8)  _daq_type = daq_type::WDB;
            else throw std::runtime_error("Error reading FILE header!");
            _file.read(header, 4);
        } 
